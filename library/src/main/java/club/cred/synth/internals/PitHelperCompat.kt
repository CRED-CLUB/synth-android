...
class PitHelperCompat(
    pitViewAppearance: PitViewAppearance,
    cornerRadius: Float,
    private val drawBackground: Boolean
) : PitHelperInterface {

    private val fillBackgroundHelper: BlurHelper = BlurHelper(
        blur = 0f,
        radius = cornerRadius,
        shadowColor = pitViewAppearance.compatFillColor
    )

    override fun draw(canvas: Canvas, height: Int, width: Int) {
        if (drawBackground) {
            fillBackgroundHelper.draw(canvas, height, width)
        }
    }
}
